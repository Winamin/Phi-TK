...
pub async fn main() -> Result<()> {
    ...
    fn resample_audio_to_96000hz(input_samples: Vec<f32>, original_sample_rate: usize) -> Vec<f32> {
    // 设置目标采样率 96000Hz 和源采样率（例如44100Hz）
    let target_sample_rate = 96000;
    let resample_ratio = target_sample_rate as f64 / original_sample_rate as f64;

    // 使用 SincFixedInOut 创建一个高质量的重采样器
    let mut resampler = SincFixedInOut::<f32>::new(
        resample_ratio,
        2, // 通道数，立体声为 2
        1024, // 每次处理的帧数，通常为 1024 或 2048
        InterpolationType::Cubic, // 插值方式，Cubic 是一种平衡质量和性能的插值
        WindowFunction::BlackmanHarris2, // 使用 Blackman-Harris 窗口提高重采样质量
    )
    .unwrap();

    // 将输入音频样本打包成通道向量
    let input_channels = vec![input_samples.clone(), input_samples];

    // 执行重采样
    let output = resampler.process(&input_channels, None).unwrap();
...
fn main() {
    // 假设有一个采样率为 44100Hz 的输入音频样本向量
    let original_sample_rate = 44100;
    let input_samples: Vec<f32> = vec![/* 音频样本数据 */];

    // 重采样到 96000Hz
    let resampled_samples = resample_audio_to_96000hz(input_samples, original_sample_rate);
