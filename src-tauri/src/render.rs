...
use ffmpeg::{self, format, software, frame};
...
async fn load_and_resample_audio(file_path: &str, target_sample_rate: u32, volume: f32) -> Result<Vec<f32>> {
    // 打开音频文件
    let mut context = format::input(file_path)?;
    let audio_stream = context.streams().best(ffmpeg::media::Type::Audio).ok_or("找不到音频流")?;
    ...
    let mut resampler = software::resampler(
        ...
        ffmpeg::util::channel_layout::STEREO,
    ...
    for (stream, packet) in context.packets() {
        if stream.index() == audio_index {
            ...
            while decoder.receive_frame(&mut frame).is_ok() {
                let pcm_frame = resampler.convert(&frame)?;

                // 获取 PCM 数据并调整音量
                for (ch, sample) in pcm_frame.plane::<f32>(0).iter().enumerate() {
                    resampled_data.push(sample * volume);
                }
            }
        }
    }
...
async fn main() -> Result<()> {
    ...
    // 加载并重采样每个音频
    let ending = load_and_resample_audio("path/to/ending.mp3", 48000, 0.8).await?;
    let sfx_click = load_and_resample_audio("path/to/click.ogg", 48000, 0.5).await?;
    let sfx_drag = load_and_resample_audio("path/to/drag.ogg", 48000, 0.5).await?;
    let sfx_flick = load_and_resample_audio("path/to/flick.ogg", 48000, 0.5).await?;

    let track_length = music.len() as f64 / 48000.0;
    let video_length = track_length + 1.0; // 自定义视频长度
    ...
    // 混合音频数据
    let mut pos = 0;
    for (i, &sample) in music.iter().enumerate() {
        output[pos + i * 2] += sample;
        output[pos + i * 2 + 1] += sample;
    }

    // 写入文件（可选）
    let mut file = File::create("output_audio.pcm")?;
